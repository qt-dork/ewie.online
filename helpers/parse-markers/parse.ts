...
export function parse(s: string): Root {
  // deno-lint-ignore prefer-const -- mutate children
  ...
  // shallow copy lol
  // deno-lint-ignore prefer-const -- i mean i guess it's a const pointer
  let pos = state.pos;
...
export function consumeParagraph(state: ParserState): Paragraph {
  // deno-lint-ignore prefer-const -- mutates children
  let node: Paragraph = {
    type: "paragraph",
    children: [],
  };
...
function nodeText(state: ParserState): Text {
  // deno-lint-ignore prefer-const -- bro i'm mutating children
  let node: Text = {
    type: "text",
    value: "",
  };
...
export function parseMetadata(state: ParserState): string[] | undefined {
  // deno-lint-ignore prefer-const -- array.push mutates variable ;-;
...
export function consumeMarker(state: ParserState): Marker | undefined {
  // deno-lint-ignore prefer-const -- Marker contents are reassigned
  let marker: Marker = {
    type: "marker",
    value: "",
  };
